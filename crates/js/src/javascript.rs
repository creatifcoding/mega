...
use emacs_sys::bindings::Ffuncall;
use emacs_sys::definitions::EmacsUint;
use emacs_sys::lisp::LispObject;
use emacs_sys::list::LispCons;
use emacs_sys::list::LispConsCircularChecks;
use emacs_sys::list::LispConsEndChecks;
...
impl Default for EmacsJsOptions {
    fn default() -> Self {
        Self {
            tick_rate: 0.25,
            ops: None,
            error_handler: emacs_sys::globals::Qnil,
            inspect: None,
            inspect_brk: None,
            use_color: false,
            ts_config: None,
...
fn is_interactive() -> bool {
    unsafe { !emacs_sys::bindings::globals.noninteractive1 }
}
...
macro_rules! make_proxy {
    ($scope:expr, $lisp:expr) => {{
        ...
        unsafe {
            if emacs_sys::bindings::globals.Vjs_retain_map == emacs_sys::globals::Qnil {
                emacs_sys::bindings::globals.Vjs_retain_map =
                    LispObject::cons($lisp, emacs_sys::globals::Qnil);
                emacs_sys::bindings::staticpro(&emacs_sys::bindings::globals.Vjs_retain_map);
            } else {
                emacs_sys::bindings::globals.Vjs_retain_map =
                    LispObject::cons($lisp, emacs_sys::bindings::globals.Vjs_retain_map);
            }
        }
...
macro_rules! unproxy {
    ($scope:expr, $obj:expr) => {{
        ...
        LispObject::from_C_unsigned(ptrstr.parse::<emacs_sys::definitions::EmacsUint>().unwrap())
    }};
}
...
macro_rules! make_reverse_proxy {
    ($scope:expr, $lisp:expr) => {{
        make_proxy!(
            $scope,
            LispObject::cons(emacs_sys::globals::Qjs_proxy, $lisp)
        )
    }};
}
...
pub fn lisp_make_finalizer(
    ...
    let result = unsafe {
        let mut bound = vec![
            emacs_sys::globals::Qjs__clear,
            emacs_sys::bindings::make_fixnum(len.into()),
        ];
        let list = emacs_sys::bindings::Flist(bound.len().try_into().unwrap(), bound.as_mut_ptr());
        let mut lambda = vec![emacs_sys::globals::Qlambda, emacs_sys::globals::Qnil, list];
        let lambda_list =
            emacs_sys::bindings::Flist(lambda.len().try_into().unwrap(), lambda.as_mut_ptr());
        emacs_sys::bindings::Fmake_finalizer(lambda_list)
    };
...
pub fn lisp_make_lambda(
    ...
    let llen = unsafe { emacs_sys::bindings::make_fixnum(len.into()) };
    ...
    let finalizer = unsafe {
        let mut bound = vec![emacs_sys::globals::Qjs__clear, llen];
        let list = emacs_sys::bindings::Flist(bound.len().try_into().unwrap(), bound.as_mut_ptr());
        let mut fargs = vec![emacs_sys::globals::Qand_rest, emacs_sys::globals::Qalpha];
        let fargs_list =
            emacs_sys::bindings::Flist(fargs.len().try_into().unwrap(), fargs.as_mut_ptr());

        let mut lambda = vec![emacs_sys::globals::Qlambda, fargs_list, list];
        let lambda_list =
            emacs_sys::bindings::Flist(lambda.len().try_into().unwrap(), lambda.as_mut_ptr());
        emacs_sys::bindings::Fmake_finalizer(lambda_list)
    };

    let mut inner = vec![emacs_sys::globals::Qjs__reenter, llen, finalizer];
    if num_args > 0 {
        inner.push(emacs_sys::globals::Qalpha);
    }

    let result =
        unsafe { emacs_sys::bindings::Flist(inner.len().try_into().unwrap(), inner.as_mut_ptr()) };
...
pub fn lisp_string(
    ...
    match c_alloc {
        Ok(cstr) => {
            let result = unsafe {
                emacs_sys::bindings::make_string_from_utf8(cstr.as_ptr(), len.try_into().unwrap())
            };
            let proxy = make_proxy!(scope, result);
            let r = v8::Local::<v8::Value>::try_from(proxy).unwrap();
            retval.set(r);
...
pub fn lisp_fixnum(
    ...
    let result = unsafe { emacs_sys::bindings::make_fixnum(message) };
    let proxy = make_proxy!(scope, result);
    let r = v8::Local::<v8::Value>::try_from(proxy).unwrap();
    retval.set(r);
}
...
pub fn lisp_float(
    ...
    let result = unsafe { emacs_sys::bindings::make_float(message) };
    let proxy = make_proxy!(scope, result);
    let r = v8::Local::<v8::Value>::try_from(proxy).unwrap();
    retval.set(r);
}
...
pub fn lisp_intern(
    ...
    let result = unsafe { emacs_sys::bindings::Fintern(lispobj, emacs_sys::globals::Qnil) };
    let proxy = make_proxy!(scope, result);
    let r = v8::Local::<v8::Value>::try_from(proxy).unwrap();
    retval.set(r);
}
...
pub fn lisp_list(
    ...
    let result = unsafe {
        emacs_sys::bindings::Flist(lisp_args.len().try_into().unwrap(), lisp_args.as_mut_ptr())
    };
    let proxy = make_proxy!(scope, result);
    let r = v8::Local::<v8::Value>::try_from(proxy).unwrap();
    retval.set(r);
...
pub fn finalize(
    ...
    let len = args.length();
    let mut new_list = LispObject::cons(emacs_sys::globals::Qnil, emacs_sys::globals::Qnil);
    for i in 0..len {
        let arg = args.get(i);
        if arg.is_object() {
            let lispobj = unproxy!(scope, arg.to_object(scope).unwrap());
    ...
    unsafe { emacs_sys::bindings::globals.Vjs_retain_map = new_list };
}
...
pub fn is_reverse_proxy(
    ...
    if args.get(0).is_object() {
        ...
        if let Some(cons) = lisp.as_cons() {
            is_reverse_proxy = cons.car().eq(emacs_sys::globals::Qjs_proxy);
        }
    }
...
pub fn make_reverse_proxy(
    ...
    let finalizer = unsafe {
        let list = list!(
            emacs_sys::globals::Qjs__clear_r,
            emacs_sys::bindings::make_fixnum(idx.into())
        );
        let lambda_list = list!(emacs_sys::globals::Qlambda, emacs_sys::globals::Qnil, list);
        emacs_sys::bindings::Fmake_finalizer(lambda_list)
    };
...
unsafe extern "C" fn lisp_handler(
    _arg1: emacs_sys::bindings::nonlocal_exit::Type,
    arg2: LispObject,
) -> LispObject {
    LispObject::cons(emacs_sys::globals::Qjs_lisp_error, arg2)
}
...
pub fn lisp_invoke(
    ...
    let results = unsafe {
        emacs_sys::bindings::internal_catch_all(
    ...
    if results.is_cons() {
        ...
        if cons.car() == emacs_sys::globals::Qjs_lisp_error {
            // Lisp has thrown, so we want to throw a JS exception.
            let lisp_error_string =
                unsafe { emacs_sys::bindings::Ferror_message_string(cons.cdr()) };
    ...
    let is_primative = unsafe {
        emacs_sys::bindings::STRINGP(results)
            || emacs_sys::bindings::FIXNUMP(results)
            || emacs_sys::bindings::FLOATP(results)
            || results == emacs_sys::globals::Qnil
            || results == emacs_sys::globals::Qt
    };
    if is_primative {
...
fn permissions_from_args(args: &[LispObject]) -> EmacsJsOptions {
    ...
    for i in 0..args.len() {
        ...
        match key {
            emacs_sys::globals::QCallow_net => {
                if value == emacs_sys::globals::Qnil {
                    permissions.net.revoke::<&str>(None);
                }
            }
            emacs_sys::globals::QCallow_read => {
                if value == emacs_sys::globals::Qnil {
                    permissions.read.revoke(None);
                }
            }
            emacs_sys::globals::QCallow_write => {
                if value == emacs_sys::globals::Qnil {
                    permissions.write.revoke(None);
                }
            }
            emacs_sys::globals::QCallow_run => {
                if value == emacs_sys::globals::Qnil {
                    permissions.run.revoke(None);
                }
            }
            emacs_sys::globals::QCjs_tick_rate => unsafe {
                if emacs_sys::bindings::FLOATP(value) {
                    options.tick_rate = emacs_sys::bindings::XFLOAT_DATA(value);
                }
            },
            emacs_sys::globals::QCjs_error_handler => {
                options.error_handler = value;
            }
            emacs_sys::globals::QCinspect => {
                if value.is_string() {
                    options.inspect = Some(value.as_string().unwrap().to_utf8());
                } else if value == emacs_sys::globals::Qt {
                    options.inspect = Some(DEFAULT_ADDR.to_string());
                }
            }
            emacs_sys::globals::QCinspect_brk => {
                if value.is_string() {
            ...
            emacs_sys::globals::QCuse_color => {
                if value.is_t() {
                    options.use_color = true;
                }
            }
            emacs_sys::globals::QCts_config => {
                let sref: LispStringRef = value.into();
                let rstring = sref.to_utf8();
                options.ts_config = Some(rstring);
            }
            emacs_sys::globals::QCno_check => {
                if value.is_t() {
                    options.no_check = true;
                }
            }
            emacs_sys::globals::QCno_remote => {
                if value.is_t() {
...
pub fn eval_js(args: &[LispObject]) -> LispObject {
    ...
    let is_typescript = args.len() == 3
        && args[1] == emacs_sys::globals::QCtypescript
        && args[2] == emacs_sys::globals::Qt;
...
pub fn eval_js_expression(args: &[LispObject]) -> LispObject {
    ...
    let mut call = vec![emacs_sys::globals::Qeval_expression, result];
    for i in 1..args.len() {
        call.push(args[i]);
    }
...
fn get_buffer_contents(mut buffer: LispObject) -> LispObject {
    if buffer.is_nil() {
        buffer = unsafe { emacs_sys::bindings::Fcurrent_buffer() };
    }

    unsafe {
        let current = emacs_sys::bindings::Fcurrent_buffer();
        emacs_sys::bindings::Fset_buffer(buffer);
        let lstring = emacs_sys::bindings::Fbuffer_string();
        emacs_sys::bindings::Fset_buffer(current);
        lstring
    }
}
...
pub fn eval_ts_buffer(buffer: LispObject) -> LispObject {
    ...
    eval_js(&[
        lisp_string,
        emacs_sys::globals::QCtypescript,
        emacs_sys::globals::Qt,
    ])
}

fn get_region(start: LispObject, end: LispObject) -> LispObject {
    let saved = unsafe { emacs_sys::bindings::save_restriction_save() };
    unsafe {
        emacs_sys::bindings::Fnarrow_to_region(start, end);
        let lstring = emacs_sys::bindings::Fbuffer_string();
        emacs_sys::bindings::save_restriction_restore(saved);
        lstring
    }
}
...
fn js_reenter_inner(scope: &mut v8::HandleScope, args: &[LispObject]) -> EmacsJsResult<LispObject> {
    ...
    if !unsafe { emacs_sys::bindings::INTEGERP(index) } {
    ...
    let value = unsafe {
        emacs_sys::bindings::check_integer_range(
            index,
            emacs_sys::bindings::intmax_t::MIN,
            emacs_sys::bindings::intmax_t::MAX,
        )
    };
    ...
    if args.len() > 2 {
        ...
        cons.iter_cars(LispConsEndChecks::on, LispConsCircularChecks::on)
            .for_each(|a| {
                let is_primative = unsafe {
                    emacs_sys::bindings::STRINGP(a)
                        || emacs_sys::bindings::FIXNUMP(a)
                        || emacs_sys::bindings::FLOATP(a)
                        || a == emacs_sys::globals::Qnil
                        || a == emacs_sys::globals::Qt
                };
                if is_primative {
...
fn execute_function_may_throw(
    ...
    let mut retval = emacs_sys::globals::Qnil;
...
fn tick_and_schedule_if_required() {
    if !EmacsMainJsRuntime::is_within_runtime() && !EmacsMainJsRuntime::get_tick_scheduled() {
        js_tick_event_loop(emacs_sys::globals::Qnil);
    }
}
...
pub fn js_get_tick_rate() -> LispObject {
    ...
    unsafe { emacs_sys::bindings::make_float(options.tick_rate) }
}
...
pub fn js_set_tick_rate(f: LispObject, loops_per_tick: LispObject) {
    ...
    unsafe {
        emacs_sys::bindings::CHECK_NUMBER(f);
        options.tick_rate = emacs_sys::bindings::XFLOATINT(f);
...
fn schedule_tick() {
    ...
    let tick_rate = unsafe { emacs_sys::bindings::make_float(js_options.tick_rate) };
    if is_interactive() {
        rate = tick_rate;
        repeat = emacs_sys::globals::Qnil;
    } else {
        rate = emacs_sys::globals::Qt;
        repeat = tick_rate;
    }

    unsafe {
        let mut args = vec![
            emacs_sys::globals::Qrun_with_timer,
            rate,
            repeat,
            emacs_sys::globals::Qjs_tick_event_loop,
        ];
        emacs_sys::bindings::Ffuncall(args.len().try_into().unwrap(), args.as_mut_ptr());
    }
}
...
#[allow(dead_code)]
fn init_syms() {
    defvar_lisp!(Vjs_retain_map, "js-retain-map", emacs_sys::globals::Qnil);
